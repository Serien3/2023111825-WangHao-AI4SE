"""抓取脚本：对每个仓库抓取 PR 及其全部子资源，缓存到 data/raw/。

缓存策略（断点续传）：每个 PR 存为 data/raw/<owner>__<repo>/pr_<number>.json。
...
用法：
    uv run python -m src.exp1.fetch                 # 全量（5 仓库）
    uv run python -m src.exp1.fetch --repo django/django --limit 5   # 小样验证
    uv run python -m src.exp1.fetch --no-ai         # 跳过 AI 定向补采
"""
