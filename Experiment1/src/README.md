...
```bash
uv run python -m src.fetch                                          # full: 5 repos × 300 closed PRs + AI oversampling (~1.5-2h)
uv run python -m src.fetch --repo django/django --limit 20 --no-ai  # small sample for testing
uv run python -m src.build_dataset                                  # raw JSON cache → 5 normalized tables
uv run python -m src.analyze                                        # EDA: stats + 7 figures
uv run python -m src.github_client                                  # smoke-test token + rate limit
```

## Pipeline

Three stages, each writing to `Experiment1/results/` (git-ignored, regenerable):

```
GitHub API ──fetch──▶ results/raw/<owner>__<repo>/pr_<n>.json ──build──▶ results/processed/*.{parquet,csv} ──analyze──▶ results/figures/*.png
```
