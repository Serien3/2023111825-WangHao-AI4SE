...
> 供后续实验（二～七）参考。代码位于 `src/exp1/`，运行命令见 `src/exp1/CLAUDE.md`。
...
每个 PR 被打包成一个 JSON 文件（`data/raw/<owner>__<repo>/pr_<编号>.json`），包含：
...
对应实验一步骤五，由 `src/exp1/analyze.py` 生成，图表保存至 `data/figures/`。
